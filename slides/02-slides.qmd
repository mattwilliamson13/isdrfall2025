...
\tikzset{
  ...
  inner/.style   = {circle, draw, thick, fill=green!15,
                    inner sep=2pt, % small fixed padding
                    align=center, font=\bfseries\large},
  boxlab/.style  = {font=\bfseries\Huge, fill=white, inner sep=2pt},
  link/.style    = {thick, -{Stealth[length=4mm,width=2.5mm]}}
}

\begin{tikzpicture}[node distance=2.5cm and 3cm]

% ---------- Ideate inner nodes ----------
\node (litrev)   [inner, fill=orange!15] {Literature Review};
\node (resquest) [inner, fill=orange!15, below=1.2cm of litrev] {Research Questions};
\draw[link] (litrev.south) -- (resquest.north);

% ---------- Ideate background ----------
\node[draw, thick, rounded corners=6pt, fill=orange!10, minimum width=6.5cm, minimum height=4.5cm] 
      (ideatebg) at ($(litrev)!0.5!(resquest)$) {};
\node[boxlab] at ($(ideatebg.north)+(0,-0.25)$) {Ideate};

% ---------- Get Data ----------
\node (getdata) [phase, right=3.5cm of ideatebg] {Get Data};

% ---------- Outer flow: Import / Tidy ----------
\node (import) [phase, right=3cm of getdata] {Import};
\node (tidy)   [phase, right=3cm of import] {Tidy};

% ---------- Understand inner cycle ----------
\node (transform) [inner, above right=1.5cm and 0.5cm of tidy] {Transform};
\node (visualize) [inner, below right=1cm and 0.5cm of transform] {Visualize};
\node (model)     [inner, below left=1cm and 0.5cm of visualize] {Model};

% ---------- Understand background ----------
\node[draw, thick, rounded corners=6pt, fill=gray!10, minimum width=6.5cm, minimum height=4.5cm] 
      (understandbg) at ($(transform)!0.5!(model)$) {};
\node[boxlab] at ($(understandbg.north)+(0,0.25)$) {Understand};

% ---------- Communicate ----------
\node (communicate) [phase, right=4cm of understandbg] {Communicate};

% ---------- Arrows ----------
% Main flow arrows in red
\draw[link, red] (ideatebg.east) -- (getdata.west);
\draw[link, red] (getdata.east) -- (import.west);
\draw[link, red] (import.east) -- (tidy.west);
\draw[link, red] (tidy.south) -- ($(transform.north)+(0,0.1)$);
\draw[link, red] (understandbg.east) -- (communicate.west);

% Inner cycle arrows (black)
\draw[link, bend left=25] (transform) to (visualize);
\draw[link, bend left=25] (visualize) to (model);
\draw[link, bend left=25] (model) to (transform);

% Curved return arrow (black) Communicate → Ideate
\draw[link, bend left=35] (communicate.north) to (ideatebg.north);
