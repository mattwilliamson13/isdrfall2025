...
%%| fig-height: 8.5
graph LR
  classDef allnodes fill:#db8872,stroke:#ffff,stroke-width:2px,font-size:60px; 
  classDef invisible fill:none,stroke:none,font-size:1px,padding:0px;

  Import[Import] --> Tidy[Tidy]
  Tidy --> EntryPoint[" "]:::invisible

  subgraph Understand[" "]
    EntryPoint --> Transform[Transform]
    Transform ---> Visualize[Visualize]
    Visualize ---> Model[Model]
    Model ---> Transform 
    Model -->  ExitPoint[" "]:::invisible
    Label["Understand"]:::label
  end

  ExitPoint --> Communicate[Communicate]

  style Understand fill:#44006540,stroke:#ffff,stroke-width:6px
  class Tidy,Import,Transform,Visualize,Model,Communicate allnodes;

  classDef label fill:none,stroke:none,font-weight:bold,font-size:24px,text-align:left;
